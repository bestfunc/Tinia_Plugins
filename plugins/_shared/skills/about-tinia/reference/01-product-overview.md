...
Tinia 不是一个单品，是面向**学生 → 工程师 → 工厂**的三层产品阶梯。

```
┌─────────────────────────────────────────────────────────────┐
│  Tier 3 ── Tinia Production（在线产线版，2027 规划）         │
│  • 实时流处理 / 多机协同 / MES 集成 / 模型 OTA               │
│  • 面向：工厂在线监测（风电场、工业泵、电机厂等）            │
│  • 定价：项目制                                              │
│  → 复用 SmartQuality 现有客户基础 + 在线检测能力              │
├─────────────────────────────────────────────────────────────┤
│  Tier 2 ── Tinia Pro（2026 H2 主交付）                       │
│  • 完整离线分析工作站 / 多通道 / Order Tracking / 报告       │
│  • 面向：NVH 工程师、研究院、Tier1 测试部门                  │
│  • 定价：按 seat 订阅 / 永久买断（双模式）                   │
│  → 这是当下唯一收入来源                                      │
├─────────────────────────────────────────────────────────────┤
│  Tier 1 ── Tinia Community（永久免费）                       │
...
- **Tier 2 Pro** 跟传统 NVH 软件相比，单 seat 价格更低，是因为云原生 + 订阅 + AI 增效让单 seat 边际成本接近零（传统是 30 年 C++ 桌面单机软件，没法做到）
- **Tier 1 Community** 是传统大厂不敢做的 —— 他们的核心客户是大企业，免费版会侵蚀付费版。Tinia 起步阶段没有存量包袱
- **Tier 3 Production** 是 SmartQuality（Bestfunc 现有产品线）客户的自然升级，2027 启动
...
Tinia 主仓直接内置 MCP server（`/api/v1/mcp`），让 Claude Code、Codex、Qwen CLI 等 AI 客户端通过 OAuth 2.1 一键授权后**直接驱动 Tinia 完成节点开发 / 流程编排 / 数据上传 / 跑流程 / 发布**全流程。
...
这个叙事是 Tinia 的战略主线。功能层面 Tinia 会持续补齐传统工具的核心算法，但产品故事一定要从**"工具替代"转向"AI 工程师 + 节点商店"** —— 这是融资 / 投资估值差距的根源，也是客户决定 5 年后用谁的根源。

### 离线 vs 在线的边界判断

- **2026**：Tinia 是**离线分析工具**，对标 HEAD/Testlab。在线监测靠 SmartQuality（Bestfunc 现有产品）
- **2027**：把 Tinia 离线引擎"在线化"，推出 Tinia Production。架构上 `tinia-engine` 跟 `tinia-runtime` 的分离就是为此准备
- 投资人买的是"产线工具"叙事（百亿+ TAM），不是"工程师工具"（中国 2-3 亿/年 TAM）。但短期必须先把离线工具卖出去回血
...
| 风电场 / 工业泵 PdM | 自家产线没有 NVH 软件能用 | Production（2027）：在线流处理 + MES 集成 | `scenarios/04-online-monitoring.md` |
...
2. **三层产品矩阵**（Community / Pro / Production）覆盖学生到工厂全谱，不要混着卖
3. **MCP-native** 是架构层面的领先，不是后期能加的功能 —— 这一点 5 年内对手追不上
4. **节点商店** 是 2027 主线之一，今年内部跑通明年开放，但今天就可以跟客户讲
5. **不要被算法清单（134 项笛卡尔积）吓住** —— HEAD 30 年才做到 134 项，Tinia 2 年到 60+ 项加 AI 编排，价值已经超过他们
